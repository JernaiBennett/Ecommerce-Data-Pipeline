...
def download_data_from_s3(s3, bucket_name):
    ...
    for file_name in data_files:
        try:
            ...
            #temporarily save files to this location
            ...
            #remove the temporary location to free up disk space
...
def transform_data(datasets):
    ...
    if 'customers' in datasets:
        customers = datasets['customers'].copy()   #best to work on data copies
        ...
        #create age groups
        customers['age_group'] = pd.cut(customers['age'], bins = [0, 25, 35, 50, 65, 100], labels = ['18-25', '26-35', '36-50', '51-65', '65+'])
    ...
    if 'products' in datasets:
        products = datasets['products'].copy()   #best to work on data copies
        ...
        products['product_price'] = pd.cut(products['price'], bins = [0,50, 150, 500, float('inf')], labels = ['budget', 'mid-range', 'premium', 'luxury'])
    ...
    if 'orders' in datasets:
        orders = datasets['orders'].copy()   #best to work on data copies

        #convert date 
        orders['order_date'] = pd.to_datetime(orders['order_date'])

        #convert total amounts to numeric values
     ...
     #Process the order items data 
    if 'order_items' in datasets:
        order_items = datasets['order_items'].copy()   #best to work on data copies
        ...
        print(f"Processed order items: {len(order_items)} records")    

     #Process reviews data 
    if 'reviews' in datasets:
        reviews = datasets['reviews'].copy()   #best to work on data copies
...
def create_business_metrics(processed_datasets):
    ...
    #extract the clean datasets and put them into new variables
    ...
    if 'products_clean' in processed_datasets and 'order_items_clean' in processed_datasets:
        ...
        #calculate products performance
        ...
        product_metrics = product_metrics.merge(products[['product_id', 'product_name', 'category', 'price']],
                                                on = 'product_id')
...
def upload_processed_data(s3, bucket_name, processed, metrics):
    ...
    for dataset_name, df in processed.items():
        try:
            ...
            print(f"Error: Failed to upload {dataset_name}: {e}")
